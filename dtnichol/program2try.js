...
for(var i=0;i<coursesPrefixes.length;i++)
{
	...
	xmlhttp[i].onreadystatechange =  // Here's where I got trapped and spent more than 80% of program time on.... SCOPING in JS is different. when you have function as array object, you must declare a returning with another index to be read correctly.
	function(index) {
        return function() {
			if ((xmlhttp[index].readyState == 4) && (xmlhttp[index].status == 200)) {
				...
				{
					pushIntoDefinedArr(jsonArr,j); // calling another function to push into courses array, just because of scoping issues.
				}
			};
		};
	}(i);
...
function prereqsTaken (taken,course) {
	//document.getElementById("coursesTaken").innerHTML = taken;
...
// function that handles submit button. all the magic goes here.
function scheduleSubmit()
{
	...
	//takes the text area input and changes it to capital letters for function to recognize and math with API prefixes
...
//note: The classes I've taken text "For testing purposes" : MATH 161, ENGL 111, ENGL 104, ENGL 204, MUS 117A, CHEM 111L, CHEM 111, BIOL 104B, HIST 100D, COMM 105A, MATH 300, MATH 223, CS 105, CS 116, CS 146, CS 216, CIS 255, MATH 120A, HUM 190, MATH 140
