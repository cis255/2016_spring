//code up to course arrays from Zaiter
//Declaring an empty array that will hold our json objects from svsu api custom made json file.
allCoursesArr = [];
...
// prerequisite list from Staci (skstockm)
var prereqs = [
	...
	for (var i=0; i <prereqs.length; i++) {
		...
		if(prereqs[i].course == course) {
			...
			if (taken.indexOf(prereqs[i].prereq1) == -1 
				...
				|| (taken.indexOf(prereqs[i].prereq3) == -1
				&& taken.indexOf(prereqs[i].prereq3 !=""))
				|| (taken.indexOf(prereqs[i].prereq4) == -1
				&& taken.indexOf(prereqs[i].prereq4 !=""))){
				retval = false;
				break;
			}
		}
...
//below code from Zaiter because I struggled farr too hard trying to figure this darn thing out
// function that handles submit button. all the magic goes here.
function scheduleSubmit()
{
	//first remove all dropdown options.
	removeOptions(document.getElementById("availableCourses"));
	...
	for(var j = 0; j < coursesForGraduationArr.length; j++)
	{
		...
		if(prereqTaken(taken,coursesForGraduationArr[j]) && takenArr.indexOf(coursesForGraduationArr[j]) == -1)
		{
	...
	for(var z=0;z<coursesEligible.length;z++)
	{
		...
		{
			// if course eligible to be taken available in the API, then push it into courses available for registeration array
	...
	for(var i = 0; i < coursesEligibleJsonArr.length; i++) {
		...
		document.getElementById("availableCourses").appendChild(newP);
